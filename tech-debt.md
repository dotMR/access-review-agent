...
- [ ] `claude-agent-sdk>=0.1.0` has no upper bound in `pyproject.toml`, for a pre-1.0 SDK this project has already hit one real silent-breakage gotcha with (see `agent_sdk.py`'s module docstring).
