...
**Wiring gaps surfaced immediately.** `run_full_reconciliation` never actually called `detect_identity_resolution` — every live run silently produced zero Identity resolution findings, no error. `monthly-report.yml` had `contents: read`, which would have 403'd the moment it tried to commit a report for real. Neither was caught by any eval fixture, since every one of them is a single-shot check against fresh data, never the same data checked twice.

**Duplicate-Issue prevention needed building, then fixing twice more.** Re-running detection against unchanged data (a normal push touching `system_hr.csv`, fanning out to all five systems) opened a second Issue for an already-open finding. The fix's own `system_of()` helper then broke the moment an Issue carried a third label (`accepted-risk`), silently producing a wrong dedup key. And accepted-risk Issues — closed, but the underlying condition still genuinely detected every run — weren't in the dedup set at all, re-surfacing a formally-reviewed finding as a brand-new Issue.

**Remediation re-check/auto-close didn't exist.** `SPEC.md` §8 and `iam-review-agent-design.md`'s "Closing the loop" both described it, `demo-timeline.md`'s own commit 12 depended on it, but Milestone 9 built Escalation and Accepted Risk and never this third lifecycle mechanic — a still-open Issue whose access had genuinely been revoked just stayed open.

**An EM-style code review, run deliberately as part of closing out this milestone**, found one Blocking gap the live trial itself hadn't exercised: no exception handling anywhere on the write path (`open_issue`, `close_issue`, `apply_label`, `add_comment`) — a transient GitHub API failure on one finding would have aborted the entire run, a bigger blast radius than a malformed input file gets. Fixed the same way every other failure category already was: isolated per-item, loud, non-fatal to the rest of the batch. The review's remaining, lower-priority findings are tracked in `tech-debt.md`.

**Three report-rendering bugs surfaced only once real multi-quarter data existed to render.** The aggregate report's Escalations-this-period table was hardcoded to a "not yet implemented" placeholder — Milestone 9 built Escalation months ago, but nothing ever wired the real data through. The quarterly Release body's own escalation count used a separate, non-period-aware computation that could (and did) disagree with the report's own table. And the Executive Summary's trend line always read "N/A, no prior period," for every quarter, because the parameter for it was never supplied by any caller — the finer-grained per-category recurrence narrative worked correctly throughout; only this one summary-level field was never wired. All three are the same lesson in different clothes: a field that renders convincingly is not the same as a field that's actually computed.

**A sequencing lesson, not a code bug.** Q1's quarterly-audit trigger fired before `demo-timeline.md`'s own seeding commits were pushed, so Q1's report rolled up pre-existing Milestone 1–9 verification-testing noise instead of the intended narrative — and because quarterly reports have no date-scoping by design (correct for real production use, where each period's audit genuinely runs once, in sequence), that contamination is now permanent across all three published Releases. Retroactively regenerating a published report against today's state was tried once, made it worse, and was abandoned as the wrong fix — a past report isn't something this system is designed to reconstruct. Documented here rather than concealed: every mechanism itself (detection, escalation, monthly gap-closing, remediation, accepted-risk, trend line, Risk Assessment, Release publishing) is genuinely proven working end-to-end; the specific Q1–Q3 story told by this trial's published Releases doesn't perfectly match `demo-timeline.md`'s intended narrative.

**The last deferred spec item closed.** Two live identity-resolution queries against real data both completed in exactly 4 turns — `SPEC.md` §3's tool-call/iteration cap is now pinned at 10 (`ClaudeAgentOptions.max_turns`), enforced in code, not just documented.

**Proves:** every mechanism this project claims — detection, grounding, Escalation, Accepted Risk, remediation auto-close, the monthly quiet-system catch, Risk Assessment with real narrative synthesis, trend lines, and a human-approved tagged Release — genuinely works against real GitHub state, not just credential-free fixtures. Ten real defects found this way never showed up in any eval case.
**Gate:** every fix above landed as its own branch → PR → CI-green → merge, each with a permanent regression guard wired into `eval.yml` (`scripts/verify_*.py`). The scratch repo carries three published, internally-consistent quarterly Releases; the demo-data sequencing gap is a documented characteristic of this trial run, not an unresolved defect in the code.
