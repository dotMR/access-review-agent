...
**Three further trials, three fresh scratch repos.** Trial 1's own sequencing mistake made its three Releases a permanent, imperfect narrative match, but every mechanism was still proven working — so validation continued in fresh repos (`-scratch-2`, `-scratch-3`, `-scratch-4`) rather than trying to fix the first. Each surfaced real defects no single-shot eval fixture could:

- **Recurrence scoring conflated "this Issue number appeared in a prior report" with "still an open problem."** `count_consecutive_periods` walked a per-system report's line-item history looking only for the Issue number, blind to what status it held each time it appeared — a finding remediated in Q2 kept scoring as "recurring" (even "reinfection... occurring," Likelihood escalating to High) in Q3. Fixed for Remediated first; a later trial then caught the identical bug still live for Accepted-risk findings, one accepted for the first time in Q2 narrated as "persisting... across two consecutive audit cycles" — a determination the record shows happened exactly once. Both terminal statuses now short-circuit to `consecutive_periods=1`; the backward-walk is only meaningful for an Issue still genuinely open.
- **The same non-scoping bug, one layer up.** Risk Assessment is specified as "one entry per (category, system) pair with at least one Finding this quarter," but the code passed every Issue ever seen for that pair, lifetime, into the scoring function — a finding closed in Q2 kept reappearing as its own row in Q3, wrongly narrated as resolved "this cycle." Took two tries: comparing against `period_bounds`' simulated calendar quarters broke immediately, since compressing three quarters into one real day closes everything with a real "today" timestamp regardless of which simulated quarter it represents; the working fix compares against the prior period's own real report-generation timestamp instead — a real, sequential boundary, correct in genuine production too.
- **The seeding tool's own batching hid two of demo-timeline.md's own scenarios from ever happening.** Steps meant to land as separate pushes (flag an isolated violation, then correct it before the audit; open an Orphaned finding, then revoke it same-day) were batched into one commit each, so push-triggered detection never saw the violation before it was already fixed. Split into separate checkpoints.
- **A GitHub Actions job checked out a stale commit.** `create-release`'s checkout defaulted to `github.sha`, pinned at the run's original trigger — predating the same run's own earlier job committing the files it needed to read, a reliable `FileNotFoundError`. Fixed by checking out `github.ref` instead, resolving to the branch's live tip.
- **A narrative synthesis call produced its own markdown, unasked.** Nothing in the prompt forbade formatting, so one Risk Assessment narrative came back with a `# Analysis` heading, which the (correct, load-bearing) table-cell escaping then rendered as literal `&#35; Analysis` garbage. Fixed by requiring plain prose in the prompt.
- **Labeling isn't closing.** `close_accepted_risk_issues` only runs during a reconciliation pass (a push or a monthly report) — an Issue freshly labeled `accepted-risk` stays open, and stays scored as such, until the next such pass genuinely runs. The seeding sequence had none between the label and the next quarterly audit; added one.
- **A policy guarantee with zero narrative proof, and no bug behind it.** Cross-referencing `access-control-policy.md` against `demo-timeline.md` directly (not against code) found "Escalation fires once" exercised by an eval case but never actually demonstrated in the timeline. The mechanism was already correct — a later reconciliation pass, unconditional over every open Issue, already re-checks an already-escalated finding and correctly skips it — the STEPS list and timeline just never said so. No code changed; both got a beat documenting what already happens.

**Proves:** every mechanism this project claims — detection, grounding, Escalation, Accepted Risk, remediation auto-close, the monthly trigger's own independent detection cadence, Risk Assessment with real narrative synthesis, trend lines, and a human-approved tagged Release — genuinely works against real GitHub state, confirmed across four separate scratch-repo trials, not just one. (Not proven, and corrected in `demo-timeline.md`: the "quiet system caught only by monthly" scenario specifically — the timeline's own technique for simulating a threshold crossing requires a commit that ordinary push detection catches first, a limitation of the seeding technique, not a gap in the monthly trigger's real value.) Trial 1 found ten real defects that never showed up in any eval case; the three later trials found seven more, including two variants of the same recurrence-scoring class of bug. The fourth trial confirmed a genuinely clean run, start to finish — every defect found across all four fixed and reverified live before its Releases were approved. Once confirmed, the same sequence was run for real one more time: against this repository itself, not a scratch copy (`docs/adr/0008-go-live-with-real-demo-data-in-main-repo.md`), producing this repo's own three published Releases and its own `WALKTHROUGH.md` — one more real defect (GitHub's Issues API returning pull requests, silently inflating the first Release's own summary counts) surfaced and got fixed in that final run, the only bug this whole arc found that a scratch repo could never have caught, since none of them ever had a real pull request of their own to collide with. The four scratch repos, having served their purpose, were deleted afterward — the live reference now lives here, not in a disposable copy.
**Gate:** every fix landed as its own branch → PR → CI-green → merge, each with a permanent regression guard in `eval.yml` (`scripts/verify_*.py`). Twelve published quarterly Releases across the four scratch trials proved the mechanism repeatedly before this repository's own three (2026-Q1 through Q3) were published for real and made permanent; trial 1's demo-data sequencing gap is documented, not concealed.
