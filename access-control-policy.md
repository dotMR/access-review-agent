...
- **Dormant Admin-level access.** Access at the `admin` level in the Role → Access Mapping (see role-access-mapping.md) that is dormant and unused for more than **90** consecutive days is treated as a risk and flagged for review.
...
- **Monthly Operational Flags** — Per-system list of findings (dormant, drift, or unapproved access) reviewed via the monthly scan, each with the affected employee, the system, and what's expected vs. what's actually granted. Framed as action items, not evidence.
