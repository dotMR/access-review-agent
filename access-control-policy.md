...
- **Monthly Operational Flags** — Per-system list of *every currently open* finding, any category (still-open orphaned access included), each with the affected employee, the system, and what's expected vs. actually granted. Sourced from findings already detected on their normal cadence, not a separate monthly pass — an informational nudge to close things out before the quarter closes, not itself an Operational-cadence review: doesn't gate Escalation, and a category's presence here doesn't give it one. Framed as action items, not evidence. [2026-09-08]
