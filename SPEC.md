...
- **Body:** Access detail + Expected per policy always, plus that category's specific fields (Orphaned: date detected, time to revoke; Dormant admin-level: last used, days dormant; Unapproved: date granted, approved-by; Identity resolution: resolution outcome, evidence cited; Drift: role-change dates). Plus one field no report table carries explicitly: **Source record** — the exact file and row the finding is based on (e.g., `data/access_aws.csv`, row matching `employee_id=E12345`). That citation is what the Grounding/citation guardrail's validation step checks before `open_issue` is allowed to run. Milestone 2's implementation cites a bare file path, not a clickable GitHub blob permalink (`.../blob/<sha>/<path>`) — not because the triggering commit SHA is unavailable (it's ordinary CI/git context, e.g. `GITHUB_SHA` in Actions or `git rev-parse HEAD` locally) but because nothing calls `open_issue` from a real triggered run yet. Revisit the citation format once Milestone 5 wires up push-triggered dispatch and a real triggering SHA is actually in scope to thread through.
