...
Reuses each category's own report-row columns (`templates/report-template-per-system.md`) rather than a separate, invented convention — the Issue *is* the evidentiary record the grounding/citation guardrail checks, not a summary of one.
...
| :-- | :-- | :-- | :-- | :-- | :-- |
| Monthly Operational Flags | `templates/report-template-monthly-flags.md` | `reports/monthly/<period>/<system>.md` | Monthly | — (informational, no sign-off) | Per-system, all currently-open Findings any category (Orphaned included, if still open); runs full detection monthly to catch quiet systems, but stays Evidentiary-free — no SLA, doesn't gate Escalation, plain commit not a PR — see ADR-0003 |
| Per-system | `templates/report-template-per-system.md` | `reports/<period>/<system>.md` | Quarterly | Asset Owner | Line-item Findings for that one system only, including "No findings" lines for zero-count categories |
| Aggregate (Quarterly Audit Report) | `templates/report-template-quarterly-audit.md` | `reports/<period>/aggregate.md` | Quarterly | Security/Compliance Reviewer | Executive summary + trend line, methodology, resolution-status rollup, Risk Assessment section, Escalations section, links out to the five per-system reports |
