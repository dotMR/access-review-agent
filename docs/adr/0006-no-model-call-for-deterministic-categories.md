...
**Update, 2026-09-15:** `reference/milestone-6-agent-sdk-patterns/` (referenced above) has since been deleted. By this point every pattern it captured had been carried forward into live code and improved on: the `ResultMessage.result` extraction into `agent_sdk.py:run_query()` (shared by `narrative.py` and `detection/identity_resolution.py`), tool isolation into `tools/access_data.py`/`tools/hris.py`'s factory functions, and `allowed_tools` scoping into `identity_resolution.py:build_options()`. The bug explanation this ADR summarizes above is preserved in full in `agent_sdk.py`'s module docstring.
