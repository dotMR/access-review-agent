...
- The Source record citation in the Issue body still cites a bare file path, not a GitHub blob permalink (`.../blob/<sha>/<path>`) — that's a separate, later gap (a triggering commit SHA has nowhere to come from until Milestone 5's push-triggered dispatch exists), not something this ADR's adapter design blocks. Tracked in `SPEC.md` §4 and `development-plan.md`'s Milestone 5.
